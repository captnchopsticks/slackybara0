...
void loop() {
  ...
  if (userInput != "^^" && userInput != "VV") {
    ...
    if (initFlag) { //initFlag dynamically handles overflow rows as opposed to a fixed checker of (int)messageHistory.size() > 7
      ...
      bulkRenderRow(messageHistory, nextBottomIndex, false);
      displayCanvasBuffer();

      nextBottomIndex++;
      Serial.print("nextBottomIndex: "); Serial.println(nextBottomIndex);
      Serial.print("nextTopIndex: "); Serial.println(nextTopIndex);
    }
    else {
  ...
  else if (nextTopIndex > -1 && userInput == "^^") { //cursorIndex conditional will prevent errors where index = -1
    //cursorIndex--;
    //startIndex = cursorIndex - 6;

    bulkRenderRow(messageHistory, nextTopIndex, true);
    displayCanvasBuffer();
  ...
  else if (nextBottomIndex < (int)messageHistory.size() && userInput == "VV") {
...
void partialRenderRow(chatHistory &userEntry, u_int16_t y) {
  display.setPartialWindow(userEntry.x, y, userEntry.txtboxWidth, userEntry.rowMultiple * rowHeight); //I want height to be in multiples of rowHeight
  //display.setTextColor(GxEPD_BLACK);
  display.firstPage();
  do {
    display.fillRect(userEntry.x, y, userEntry.txtboxWidth, rowHeight * userEntry.rowMultiple, GxEPD_BLACK);
...
void bulkRenderRow(std::vector<chatHistory> &myHistory, int startIndex, bool biasTOP) {
  int cursor = 0;
  canvas.fillScreen(GxEPD_WHITE);
  ...
  if (biasTOP) { //biasTOP generates from top to bottom, the next entries are aligned to top
    ...
    while (y < 114) {
      ...
      canvas.fillRect(renderEntry.x, y, renderEntry.txtboxWidth, rowHeight * renderEntry.rowMultiple, GxEPD_BLACK);
